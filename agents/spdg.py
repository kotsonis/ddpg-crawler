...
class SDPGAgent(Agent):
    ...
    def _learn_step(self):
        ...
        q_target_tile = q_target_sorted.unsqueeze(-2).repeat(1,num_atoms,1) # [B,num_atoms,num_atoms]
        q_online_tile = q_online_sorted.unsqueeze(-1).repeat(1,1,num_atoms) # [B,num_atoms,num_atoms]
        ...
        if (self.hasPER):
            with torch.no_grad():
                ...
                td_error =  td_error.view(self.sample_size,-1).mean(dim=1).abs()
